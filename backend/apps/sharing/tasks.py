...
def recover_stuck_files(older_than_minutes: int = 5) -> dict:
    """
    Requeue every file that is stuck in SCANNING or PENDING status.

    Safe to call directly from the Django shell — does NOT go through
    Celery task machinery, so it works even when the task registry
    hasn't fully initialised:

        >>> from apps.sharing.tasks import recover_stuck_files
        >>> recover_stuck_files()          # default: stuck > 5 min
    ...
    print(f'recover_stuck_files: found {total} stuck file(s) '
          f'(older than {older_than_minutes} min)')
    ...
    for file_obj in stuck:
        try:
            ...
            logger.exception(
                'recover_stuck_files: dispatch failed file_id=%s',
                file_obj.id,
            )
